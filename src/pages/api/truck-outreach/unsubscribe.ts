...
async function unsubscribe(token: string | null) {
  ...
  const { data } = await supabaseAdmin.from("truck_profile_outreach").select("email").eq("tracking_token", token).maybeSingle();
...
export async function GET({ url }: { url: URL }) {
  ...
  return new Response(`<!doctype html><html><body style="font-family:Arial;padding:48px;color:#0f172a"><h1>${ok ? "You have been unsubscribed." : "This unsubscribe link is invalid."}</h1><p>${ok ? "We will not send further truck profile outreach to this email address." : "No preferences were changed."}</p></body></html>`, { headers: { "Content-Type": "text/html; charset=utf-8" } });
}
