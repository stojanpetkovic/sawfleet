...
export async function GET({ url }: { url: URL }) {
  ...
  if (token && supabaseAdmin) {
    const { data } = await supabaseAdmin.from("permit_outreach_events").select("id,status,opened_at").eq("tracking_token", token).maybeSingle();
    if (data && !["bounced", "complained", "unsubscribed"].includes(data.status)) {
      await supabaseAdmin.from("permit_outreach_events").update({
        ...
        opened_at: data.opened_at || new Date().toISOString(), updated_at: new Date().toISOString(),
      }).eq("id", data.id);
    }
  }
